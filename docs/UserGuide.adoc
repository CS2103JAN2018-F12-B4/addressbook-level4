...
[NOTE]
====
* This command locates relevant persons with the specified information and display their address on Google Maps.
* Name, phone, email, address and tags are searched by default (no `SPECIFIER` or `-all` `SPECIFIERS`)
...
_Locates the person with phone number "91234567" and display his address on Google Maps._
====
***
